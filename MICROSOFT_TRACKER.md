...
>
> **Last updated:** 2026-08-04 (post-purge audit: refreshed from currently open GitHub issues/PRs and rebuilt from PR #49126 format)
...
| -------- | ------ | --- | ----- | ------ | --------- |
| MS Teams (channel plugin) | 11 | 30 | 41 | 0 | 41 |
| Windows platform | 43 | 21 | 64 | 0 | 64 |
| WSL | 1 | 0 | 1 | 0 | 1 |
| Azure | 10 | 9 | 19 | 0 | 19 |
| SharePoint / M365 | 0 | 0 | 0 | 0 | 0 |
| **Total** | **65** | **60** | **125** | **0** | **125** |
...
| [ ] | P0 | #115367 | Provider-owned read gate requires `origin: bundled`, but every privileged chat surface (slack/discord/matrix/msteams/feishu) now ships as an external plugin → reads locked to current conversation | `P1` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `clawsweeper:source-repro` `impact:security` +1 |  |
...
| [ ] | P1 | #67177 | [msteams] Inbound file attachments silently fail in DMs — file.download.info downloadUrl not rewritten to Graph shares endpoint | `P1` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `impact:message-loss` `issue-rating: 🦪 silver shellfish` |  |
...
| [ ] | P2 | #94939 | [Bug]: 6.x state migration leaves channel conversation-store SQLite empty (0 bytes) — orphans references, breaks proactive (Bot Framework) sends (MS Teams) | `P1` `clawsweeper:no-new-fix-pr` `clawsweeper:source-repro` `clawsweeper:linked-pr-open` `impact:data-loss` `impact:message-loss` +1 |  |
...
| [ ] | P2 | #93288 | feat(msteams): per-call topLevel override on send action for proactive new channel threads | `P2` `clawsweeper:source-repro` `impact:message-loss` `issue-rating: 🦞 diamond lobster` |  |
| [ ] | P2 | #81084 | [Feature]: MSTeams channel-bound agents need opt-out from per-thread sessions | `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `impact:session-state` `issue-rating: 🌊 off-meta tidepool` |  |
...
| [ ] | P0 | #115301 | fix(msteams): resolve approvals before agent queue | L |  |
| [ ] | P0 | #114492 | fix: repair full-stack QA, gateway, and channel regressions | XL |  |
| [ ] | P0 | #111121 | Durable Core PR 2/6: add the opt-in shared-state foundation | XL |  |
...
| [ ] | P1 | #117935 | fix(ci): unblock full release validation | XL | @vincentkoc |
...
| [ ] | P1 | #107375 | Durable Core PR 1/6: define the residual-gap RFC | XL |  |
| [ ] | P1 | #106923 | fix(msteams): keep delegated auth healthy when an expired token can auto-refresh | XS |  |
| [ ] | P1 | #104690 | fix(msteams): reset sessions on app removal lifecycle | XL | @steipete |
...
| [ ] | P2 | #118062 | fix(channels): preserve command privacy in progress drafts | S |  |
| [ ] | P2 | #116647 | fix(channels): preserve bundled plugin delivery and provider contracts | M |  |
| [ ] | P2 | #112078 | feat: add NVIDIA Nemotron Speech ASR and TTS | XL | @jacobtomlinson |
...
| Resolved? | Priority | # | Title | Labels | Assignee |
| --------- | -------- | - | ----- | ------ | -------- |
| [ ] | P0 | #116240 | [Bug]: Unix process-tree kill leaves orphan grandchildren when detached:false (Windows taskkill /T has no Unix equivalent) | `P1` `clawsweeper:no-new-fix-pr` `clawsweeper:fix-shape-clear` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `clawsweeper:needs-security-review` +5 |  |
| [ ] | P0 | #114208 | [Bug]: Windows sandbox diagnostics emit a malformed container path hint | `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:source-repro` `clawsweeper:linked-pr-open` `issue-rating: 🦞 diamond lobster` `impact:ux-friction` +2 |  |
| [ ] | P0 | #114207 | [Bug]: Windows misclassifies same-path sandbox binds as aliases | `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:source-repro` `clawsweeper:linked-pr-open` `issue-rating: 🦞 diamond lobster` `impact:other` +1 |  |
...
| [ ] | P0 | #83890 | Windows restart script builds ProcessStartInfo.Arguments via string concatenation without quoting embedded double-quotes | `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-product-decision` `clawsweeper:needs-security-review` `impact:security` `issue-rating: 🦪 silver shellfish` +1 |  |
| [ ] | P1 | #112711 | [Bug]: Windows Hub node mode gets stuck in approval/repair loop and falls back to invalid bootstrap token | `bug` `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-info` `impact:auth-provider` +1 |  |
...
| [ ] | P1 | #105528 | exec/read tools silently return empty output on Windows (v2026.6.x regression) | `bug` `docs` `P1` `clawsweeper:needs-info` `issue-rating: 🦐 gold shrimp` `impact:other` +1 |  |
| [ ] | P1 | #102755 | [Bug]: The project won't start on Windows and WSL. | `bug` `bug:behavior` `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` +3 |  |
| [ ] | P1 | #91675 | fetch failed / UND_ERR_SOCKET on Windows WSL when connecting to Google Gemini | `bug` `bug:crash` `P1` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` +4 |  |
| [ ] | P1 | #91144 | [Bug]: Windows native CLI gateway Scheduled Task does not stay running; foreground window worksWindows native CLI gateway Scheduled Task does not stay running; foreground window works | `bug` `no-stale` `bug:behavior` `P2` `clawsweeper:fix-shape-clear` `clawsweeper:queueable-fix` +3 |  |
| [ ] | P1 | #90548 | macOS: per-port lsof port-health polling can saturate launchservicesd and trigger a WindowServer watchdog reboot | `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `clawsweeper:needs-live-repro` `impact:crash-loop` +3 | @vincentkoc |
...
| [ ] | P1 | #87136 | compaction: absolute token thresholds break when switching models with different context windows | `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `clawsweeper:source-repro` `impact:session-state` +2 |  |
...
| [ ] | P1 | #63491 | [Bug]: Windows Scheduled Task gateway restart/health becomes inconsistent after ready | `P1` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-info` `impact:session-state` `impact:crash-loop` +1 | @vincentkoc |
| [ ] | P2 | #117644 | [Bug]: Agent emits Unix commands (head, tilde expansion) in PowerShell on Windows - broader than #10868 | `no-stale` `P2` `clawsweeper:fix-shape-clear` `clawsweeper:queueable-fix` `clawsweeper:source-repro` `issue-rating: 🦞 diamond lobster` +1 |  |
| [ ] | P2 | #115430 | [Bug]: Corepack fails with EPERM creating pnpm shim in C:\Program Files\nodejs on Windows | `bug` `bug:behavior` `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` +3 |  |
...
| [ ] | P2 | #93081 | [Bug]: Ctrl+C not working in Windows install on foreground | `bug` `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `issue-rating: 🦪 silver shellfish` `impact:other` |  |
...
| [ ] | P2 | #40694 | Browser-opened temporary tabs/windows should close automatically after task completion | `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `impact:session-state` `issue-rating: 🌊 off-meta tidepool` +1 |  |

### Feature Requests

| Resolved? | Priority | # | Title | Labels | Assignee |
| --------- | -------- | - | ----- | ------ | -------- |
| [ ] | P0 | #89223 | [Bug]: SecretRef file provider broken on Windows 11 26200 — icacls /sid unsupported, preflight validator ignores allowInsecurePath | `P1` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `clawsweeper:needs-security-review` `clawsweeper:source-repro` +3 |  |
| [ ] | P0 | #72595 | [Feature]: Feishu channel needs per-channel proxy bypass for mixed Windows proxy setups | `enhancement` `P1` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `clawsweeper:needs-security-review` +6 |  |
...
| [ ] | P1 | #119052 | fix(daemon): keep the Windows gateway running after the foreground window closes | M |  |
...
| [ ] | P1 | #114900 | fix: decode Windows bash session output | S | @vincentkoc |
| [ ] | P1 | #95982 | fix(json-parse): exclude code-context tails from Windows-path heuristic (#93139) | S |  |
| [ ] | P2 | #119196 | fix(chutes): report accurate context windows for sparse catalog rows | XS |  |
| [ ] | P2 | #119095 | fix(ci): retry interrupted Windows dashboard release checks | S |  |
...
| [ ] | P2 | #111902 | fix(codex): stop timed-out resume process trees on Windows | S |  |
...
| [ ] | P2 | #111624 | fix(tools): read POSIX drive paths on Windows | M | @vincentkoc |
...
| [ ] | P2 | #110877 | fix(scripts): use direct-run helper for Windows guards | S | @vincentkoc |
...
| Resolved? | Priority | # | Title | Labels | Assignee |
| --------- | -------- | - | ----- | ------ | -------- |
| [ ] | P1 | #116731 | Cron failureAlert silently swallowed by quiet exits and misses WSL2 drvfs mount failure | `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `clawsweeper:needs-live-repro` `issue-rating: 🐚 platinum hermit` +1 |  |
...
| [ ] | P1 | #80926 | Azure OpenAI Responses stalls before first event when memory tools are exposed | `maintainer` `P1` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-live-repro` `impact:auth-provider` +2 |  |
| [ ] | P2 | #103067 | Centralize chat-session naming; define subagent session lifetime & cross-channel persistence | `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `clawsweeper:source-repro` `impact:session-state` +2 |  |
...
| [ ] | P1 | #117593 | [Feature]: Improve Model Override Safety, Auditing, and Centralized Management | `enhancement` `P3` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `impact:auth-provider` +1 |  |
...
| [ ] | P1 | #110299 | docs(providers): add Azure OpenAI setup page and directory entry | XS |  |
| [ ] | P1 | #108482 | refactor(skills): centralize internal write paths | L |  |
| [ ] | P1 | #98259 | fix(openai): enable prompt cache keys for Azure | L |  |
| [ ] | P2 | #117327 | refactor(telegram): centralize text delivery | XL | @steipete |
...
| MS Teams (channel plugin) | pr | P0 | #111121 | Durable Core PR 2/6: add the opt-in shared-state foundation |
...
| MS Teams (channel plugin) | pr | P1 | #117935 | fix(ci): unblock full release validation |
...
| MS Teams (channel plugin) | pr | P1 | #107375 | Durable Core PR 1/6: define the residual-gap RFC |
| MS Teams (channel plugin) | pr | P1 | #106923 | fix(msteams): keep delegated auth healthy when an expired token can auto-refresh |
| MS Teams (channel plugin) | pr | P1 | #104690 | fix(msteams): reset sessions on app removal lifecycle |
...
| Windows platform | issue | P1 | #112711 | [Bug]: Windows Hub node mode gets stuck in approval/repair loop and falls back to invalid bootstrap token |
...
| Windows platform | pr | P1 | #119052 | fix(daemon): keep the Windows gateway running after the foreground window closes |
...
| Windows platform | pr | P1 | #114900 | fix: decode Windows bash session output |
...
| Azure | issue | P1 | #117593 | [Feature]: Improve Model Override Safety, Auditing, and Centralized Management |
...
| Azure | pr | P1 | #110299 | docs(providers): add Azure OpenAI setup page and directory entry |
