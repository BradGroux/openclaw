...
>
> **Last updated:** 2026-07-29 (post-purge audit: refreshed from currently open GitHub issues/PRs and rebuilt from PR #49126 format)
...
| ------------------------- | ------ | ------ | ------- | ------ | --------- |
| MS Teams (channel plugin) | 14     | 42     | 56      | 0      | 56        |
| Windows platform          | 47     | 19     | 66      | 0      | 66        |
| WSL                       | 1      | 0      | 1       | 0      | 1         |
| Azure                     | 10     | 6      | 16      | 0      | 16        |
| SharePoint / M365         | 0      | 0      | 0       | 0      | 0         |
| **Total**                 | **72** | **67** | **139** | **0**  | **139**   |
...
| --------- | -------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| [ ]       | P0       | #115410 | security audit silently skips DM policy for channels without resolveDmPolicy (msteams, feishu)                                                                                                      | `bug` `security` `maintainer`                                                                                                                              |           |
| [ ]       | P0       | #105322 | Teams: local multi-user Workspaces sharing with exact tab RBAC                                                                                                                                      | `stale` `P3` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `clawsweeper:needs-security-review` +3 |           |
| [ ]       | P1       | #113351 | [Bug]: Duplicate image response in msteams when agent runtime is set to openclaw                                                                                                                    | `bug` `regression` `P1` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-live-repro` +2                                |           |
| [ ]       | P1       | #112368 | [Bug]: Content created with non-unique file name causes msteams to show wrong file                                                                                                                  | `bug` `regression` `clawsweeper-recovery-stuck`                                                                                                            | @steipete |
...
| [ ]       | P1       | #101049 | [Bug]: msteams plugin npm install can leave an empty dependency tree — plugin loads, provider dies silently, Microsoft sees delivery errors                                                         | `stale` `clawsweeper:needs-live-repro` `impact:message-loss` `P0` `issue-rating: 🐚 platinum hermit` `maturity:stable` +1                                  |           |
...
| [ ]       | P2       | #115367 | Provider-owned read gate requires `origin: bundled`, but every privileged chat surface (slack/discord/matrix/msteams/feishu) now ships as an external plugin → reads locked to current conversation |                                                                                                                                                            |           |
...
| [ ]       | P0       | #114901 | merge pr #7                                                                                | XL   |                 |
| [ ]       | P0       | #114492 | fix: repair full-stack QA, gateway, and channel regressions                                | XL   |                 |
| [ ]       | P0       | #112828 | feat(claws): add guided Control UI lifecycle                                               | XL   |                 |
...
| [ ]       | P2       | #115823 | refactor(msteams): decompose inbound message handler                                       | XL   | @vincentkoc     |
...
| [ ]       | P2       | #115567 | Michaelhughes2501 patch 6                                                                  | M    |                 |
| [ ]       | P2       | #115404 | fix: restore trustworthy end-to-end QA and cross-channel delivery                          | XL   |                 |
| [ ]       | P2       | #115301 | fix(msteams): resolve approvals before agent queue                                         | M    |                 |
| [ ]       | P2       | #115067 | fix(azure-speech): keep valid voices from malformed catalogs                               | S    |                 |
...
| [ ]       | P1       | #112504 | [Bug]: clawrent channel sessions fail to persist on EVERY write (Windows, temp-file+rename over open handle) — drops replies                                                            | `bug` `bug:behavior`                                                                                                                                                             |             |
...
| [ ]       | P1       | #93081  | [Bug]: Ctrl+C not working in Windows install on foreground                                                                                                                              | `bug` `stale` `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-info` +2                                                                 |             |
...
| [ ]       | P1       | #90548  | macOS: per-port lsof port-health polling can saturate launchservicesd and trigger a WindowServer watchdog reboot                                                                        | `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:fix-shape-clear` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `clawsweeper:needs-live-repro` +2      | @vincentkoc |
...
| [ ]       | P1       | #74378  | [Bug]: OpenClaw CLI commands remain alive as node.exe processes after execution on Windows                                                                                              | `bug` `regression` `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `impact:crash-loop` +2                                                                 |             |
...
| [ ]       | P2       | #111900 | Codex CLI resume timeouts can leave descendant processes running on Windows                                                                                                             | `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:linked-pr-open` `clawsweeper:needs-live-repro` `issue-rating: 🐚 platinum hermit` `impact:other` +1                                |             |
...
| [ ]       | P2       | #106203 | Remote Windows node is connected and system.which works, but Codex/WebChat exposes no node_exec surface                                                                                 | `stale` `P2` `issue-rating: 🦪 silver shellfish` `impact:ux-friction`                                                                                                            |             |
...
| [ ]       | P2       | #7057  | Flaky tests on Windows/WSL: timeouts and ENOENT in pi-tools workspace-paths & safe-bins                                            | `enhancement` `P3` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:linked-pr-open` `clawsweeper:needs-info` +3                                    |          |
...
| [ ]       | P2       | #115399 | fix(mxc): agent tools fail in Windows workspace subdirectories                   | XS   |          |
| [ ]       | P2       | #114925 | fix(ai): remove Windows path heuristic that double-escapes JSON newlines         | S    |          |
| [ ]       | P2       | #114900 | fix: decode Windows bash session output                                          | XS   |          |
...
| [ ]       | P2       | #111596 | fix(ui): keep Windows catalog sessions in one project group                      | S    |          |
| [ ]       | P2       | #111523 | fix(json-parse): exclude code-context tails from Windows-path heuristic (#93139) | S    |          |
...
| [ ]       | P2       | #113446 | Embedded session fence: false EmbeddedAttemptSessionTakeoverError on network filesystems (SMB/CIFS/Azure Files) — stat fingerprint mtimeNs/ctimeNs/ino flaps for unchanged files | `P1` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `clawsweeper:needs-live-repro` `impact:session-state` +2 |          |
| [ ]       | P2       | #111386 | Azure OpenAI embedding provider fails — api-version not forwarded as URL query parameter                                                                                         |                                                                                                                                                                      |          |
| [ ]       | P2       | #103067 | Centralize chat-session naming; define subagent session lifetime & cross-channel persistence                                                                                     | `stale` `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `clawsweeper:needs-live-repro` +4                |          |
...
| [ ]       | P1       | #102907 | Azure OpenAI Responses throws 400 when prompt_cache_key is sent to endpoints that do not support it | `stale` `P2` `clawsweeper:no-new-fix-pr` `clawsweeper:needs-maintainer-review` `clawsweeper:needs-product-decision` `clawsweeper:needs-live-repro` +2                |          |
...
| [ ]       | P2       | #108482 | refactor(skills): centralize internal write paths                              | L    |          |
...
| MS Teams (channel plugin) | pr    | P0       | #114901 | merge pr #7                                                                                                                        |
| MS Teams (channel plugin) | pr    | P0       | #114492 | fix: repair full-stack QA, gateway, and channel regressions                                                                        |
| MS Teams (channel plugin) | pr    | P0       | #112828 | feat(claws): add guided Control UI lifecycle                                                                                       |
...
| Windows platform          | issue | P1       | #112504 | [Bug]: clawrent channel sessions fail to persist on EVERY write (Windows, temp-file+rename over open handle) — drops replies                                                            |
...
| Windows platform          | issue | P1       | #93081  | [Bug]: Ctrl+C not working in Windows install on foreground                                                                                  |
| Windows platform          | issue | P1       | #86031  | [Bug]: Windows gateway listens but local health/status time out after Telegram polling stall (v2026.5.20)                                   |
| Windows platform          | issue | P2       | #106203 | Remote Windows node is connected and system.which works, but Codex/WebChat exposes no node_exec surface                                     |
